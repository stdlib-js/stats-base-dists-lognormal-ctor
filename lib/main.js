...
function LogNormal() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( mu ) || isnan( mu ) ) {
			throw new TypeError( format( '1598p,HM', mu ) );
		}
		if ( !isPositive( sigma ) ) {
			throw new TypeError( format( '1597c,HN', sigma ) );
		}
	} else {
		mu = 0.0;
		sigma = 1.0;
	...
	defineProperty( this, 'mu', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '1598d,H8', value ) );
			}
			mu = value;
		}
	});
	defineProperty( this, 'sigma', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '1598k,HE', value ) );
			}
			sigma = value;
		}
	});
